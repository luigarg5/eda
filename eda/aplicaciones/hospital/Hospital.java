...
/**
 * La clase Hospital consiste, basicamente, en un array de camas con sus pacientes asignados. 
 *        
...
public class Hospital {
    ...
    private int ingresados;         // Numero total de pacientes que han sido ingresados
    private int altas;              // Numero total de pacientes que se han curado
    private int fallecidos;         // Numero total de pacientes que han fallecido
    private int ocupacionMaxima;    // Maximo numero de camas ocupadas al mismo tiempo
    ...
    /**
     * SII hayCamasLibres(): ingresa al paciente p en la primera cama libre de un Hospital
    ...
    public void ingresarPaciente(Paciente p) {
        ...
        for (int i = 0; i < camas.length; i++)
            if (camas[i] == null || camas[i].getEstado() == Paciente.FALLECIDO) { camas[i] = p; break; }
    }
    
    /**
     * Aplica una cura a un paciente de un Hospital. Con un 95% de probabilidades el paciente
     * mejora un grado, mientras que un 5% de las veces el paciente no responde al tratamiento
     * y fallece.
     * Tanto si el paciente se cura como si fallece deja libre la cama que estaba ocupando
     * @param p Paciente al que aplicar la cura
     */
    public void aplicarTratamiento(Paciente p) {   
        p.aplicarCura();
        if (p.getEstado() == Paciente.FALLECIDO || p.getEstado() == Paciente.SANO) {
            if (p.getEstado() == Paciente.SANO) {
                altas++;
                for (int i = 0; i < camas.length; i++)
                    if (camas[i] == p) { camas[i] = null; break; }
            } 
	        else fallecidos++;
            ocupadas--;            
        }
     ...
     * @return int numero total de pacientes que han sido ingresados en un Hospital
     */
     ...
     * @return int numero total de pacientes que han sido curados en un Hospital
     */
     ...
     * @return int numero total de pacientes que han fallecido en un Hospital
     */
     ...
     * @return double porcentaje maximo de la capacidad de un Hospital que ha sido 
     * utilizada en algun instante
     */
